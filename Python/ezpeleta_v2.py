...
def path_conflict(t_idle,t_analizar,flag_idle,plazas_sifon_complemento,matriz_pre,matriz_pos):
    ...
    for tt in range (0,len(trans_idle)):
        ...
        if(cont_t_invariante>=2):
           # print("Transicion iddle que esta en dos T:",tt)
            path_conflict(trans_idle[tt],trans_idle[tt],1,plazas_sifon_complemento,matriz_pre,matriz_pos) #El 1 indica que es flag_idle
...
if(analisis=="2" or analisis=="1"):
...
else:   #se obtienen los supervisores
    ...
    for ii in range(cantidad_transiciones):
        if(matriz_es_tr[0][ii]!=-1):
            trans_idle.append(ii)
            print("Transicion Idle: ",ii+1) #+1 Por problemas de indice en petrinator empieza en 1
    ...
    for i in range(len(t_invariant_red_original)):
            aux_pos.append(t_invariant_red_original[i].split(" "))

    aux_pos = np.delete(aux_pos,cantidad_transiciones,1)

    t_invariant_red_original = aux_pos
    ...
    for i in range(len(t_conflict_red_original)):
        if(t_conflict_red_original[i]!=""):
            aux_pos.append(t_conflict_red_original[i].split(" "))

    t_conflict_red_original= aux_pos
